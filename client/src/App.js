...
function App() {
  ...
  const [locationFilter, setLocationFilter] = useState(''); // Initialize location filter state

  // Hardcoded events based on text provided by user
  const events = [
    // April 10, 2025
    {
      id: 1,
  ...
  const handleDateChange = (event) => {
    setSelectedDate(event.target.value); // Update date state
  };
  ...
  // Filter events based on selected date, search term, AND location filter
  const filteredEvents = events.filter(event => {
    // Date filtering logic
    const eventDate = event.time ? event.time.substring(0, 10) : null; // Extract YYYY-MM-DD
    ...
    return dateMatch && searchMatch && locationMatch; // Event must match all filters
  });
      ...
      <main className="main-content">
        <aside className="filters-sidebar">
          ...
          <div className="filter-group">
            ...
            />
            {/* Optional: Button to clear date filter */}
            {selectedDate && (
              <button 
